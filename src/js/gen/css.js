...
export function css(p,px,blocks){
    ...
    const font=`${th.fontName?`"${th.fontName}",`:''}"Segoe UI","Microsoft YaHei",sans-serif`;
    const has=t=>blocks.some(b=>b.type===t&&b.enabled);
...
.${px}-wrap{max-width:640px;margin:0 auto;padding:18px;color:${cText};border-radius:${th.radius}px;overflow:hidden;position:relative}
`;
    ...
    if(d&&d.enabled){
      ...
      else if(d.bgType==='image')bg=`#0e0e14 url("${String(d.bgImage||'').replace(/"/g,'%22').replace(/\\/g,'/')}") center/cover no-repeat`;
      ...
      const patTxt=d.pattern==='custom'?(d.patternText||''):PATTERNS[d.pattern];
...
`;
        ...
        case 'profile':
          s+=`
${bk} .${px}-profile{display:flex;gap:14px;align-items:flex-start;background:rgba(255,255,255,.06);border:1px solid ${th.primary}66;border-radius:${th.radius}px;padding:14px;margin:12px 0}
...
`;
          ...
          s+=`
${bk} .${px}-profc{background:rgba(255,255,255,.06);border:1px solid ${th.primary}66;border-radius:${th.radius}px;padding:12px 14px;margin:12px 0}
...
`;
        ...
        case 'qa':
          s+=`
${bk} .${px}-qa{font-size:13px;padding:8px 0}
...
${bk} .${px}-qa-body{overflow:hidden;max-height:0;transition:max-height .3s ease;padding:0}
...
${bk} .${px}-qa-item[open] .${px}-qa-a{max-height:600px;padding:0 14px 12px}
`;
          break;
      }
    });
